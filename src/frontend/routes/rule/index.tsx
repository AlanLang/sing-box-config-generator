...
function RouteComponent() {
  ...
  const { data: usageData, isFetching: isCheckingUsage } =
    useResourceUsageCheck(pendingDeleteUuid || "", "rule", !!pendingDeleteUuid);
  ...
  const handleCopy = (rule: { uuid: string; name: string; json: string }) => {
    setIsCreating(true);
    setSelectedUuid(null);
    setEditName(`${rule.name}-复制`);
    setEditJson(rule.json);
    setEditUuid(uuidv4());
    setFocusMode(true);
  };
  ...
  return (
    ...
    >
      ...
      <AlertDialog open={deleteDialogOpen} onOpenChange={setDeleteDialogOpen}>
        <AlertDialogContent>
          <AlertDialogHeader>
            <AlertDialogTitle>删除 Rule</AlertDialogTitle>
            <AlertDialogDescription>
              确定要删除 "{selectedRule?.name}" 吗？此操作无法撤销。
            </AlertDialogDescription>
          </AlertDialogHeader>
          <AlertDialogFooter>
